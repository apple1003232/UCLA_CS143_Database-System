...
<?php
	...
	//	Director basic information
	$query = sprintf("SELECT * FROM Director WHERE id = %d", $did);
	...
	$query = sprintf("SELECT m.id, m.title FROM Director d, MovieDirector md, Movie m
			WHERE d.id = %d AND md.did=d.id AND md.mid=m.id", $did);

	if($rs = mysql_query($query,$db_link)){	
			while($row = mysql_fetch_row($rs)){				
				echo "<b>Title: </b>";
				echo "<a href='showMovie.php?mid=$row[0]'>$row[1] </a><br>";
			}
			echo "<br><br><br>";
	}
	...
	$query = sprintf("SELECT m.id, m.title, ma.role FROM MovieActor AS ma, Movie AS m 
			WHERE ma.did = %d AND ma.mid = m.id", $did);
